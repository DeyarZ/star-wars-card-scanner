...
class AlternativeVisionService {
    ...
    // MARK: - Enhanced OCR with specific Yu-Gi-Oh patterns
    ...
    private func isLikelyCardName(_ text: String) -> Bool {
        ...
        let excludedPhrases = [
            "SPELL CARD", "TRAP CARD", "EFFECT", "FUSION", "SYNCHRO", "XYZ",
    ...
    private func detectCardFeatures(from image: UIImage, completion: @escaping (CardFeatures?) -> Void) {
        ...
        // Detect star level (for monster cards)
    ...
    private func detectCardType(from ciImage: CIImage) -> CardType {
        ...
        else if isGreen(borderColors) { return .spell }
        else if isPink(borderColors) { return .trap }
